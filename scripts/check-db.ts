...
async function checkDatabase() {
  try {
    ...
    const tableNames = tables.map((row: any) => 
      typeof row === 'string' ? row : (row.table_name || row[0] || Object.values(row)[0])
    );
    
    tableNames.forEach((name: string) => console.log(`  - ${name}`));
    
    // Check if Better Auth core tables exist
    const requiredTables = ['user', 'session', 'account', 'organization', 'member'];
    ...
    const customTables = ['students', 'teachers', 'groups', 'organization_metadata', 'organization_members'];
    const missingCustom = customTables.filter(t => !existingTables.some((et: string) => et.toLowerCase() === t.toLowerCase()));
