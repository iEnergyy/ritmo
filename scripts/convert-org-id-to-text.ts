		...
		for (const tableName of tables) {
			...
			try {
				...
				if (error.message.includes("violates foreign key constraint")) {
					console.log(
						`     ⚠️  Some organization_id values don't match organization.id`,
					);
...
if (ENABLE_MIGRATION) {
	...
	console.log(
		"   To run this migration, set ENABLE_DB_MIGRATION=true environment variable",
	);
	console.log(
		"   Example: ENABLE_DB_MIGRATION=true pnpm tsx scripts/convert-org-id-to-text.ts",
	);
